...
void close_websocket(struct lws *wsi)
{
    if (wsi)
    {
        unsigned char reason[LWS_PRE + 1];
        reason[LWS_PRE] = static_cast<unsigned char>(LWS_CLOSE_STATUS_NORMAL);
        lws_close_reason(wsi, LWS_CLOSE_STATUS_NORMAL, reason, sizeof(reason));
        lws_cancel_service(context); // Ensure the service loop exits
    }
}
...
void handle_signal(int signal)
{
    ...
    close_websocket(wsi); // Close the WebSocket connection
